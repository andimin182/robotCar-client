...
private:
    ...
    void servo_camera_command_callback(const custom_interface_msgs::msg::CoG::SharedPtr CoGTarget)
    {
        ...
        //=========================================================================================
        // Compute error in pixels with respect to the image plane center
        double errX_pix = CoGTarget->position_x - imageWidth_/2;
        double errY_pix = CoGTarget->position_y - imageHeight_/2;
        std::cout << "Error on X axis is: " << errX_pix << std::endl; 
        std::cout << "Error on Y axis is: " << errY_pix << std::endl;

        //=========================================================================================
        // Servo camera control
        //=========================================================================================
        /* it would be nice to implement a Image based Visual servo control but the problem
         is that is difficult to estimate the depth Z of a point. This serves to compute then the Jacobian matrix*/

        /* Servo 1 */
        if(std::abs(errX_pix) > errTol) {
            if(errX_pix > 0){
        ...
        //=========================================================================================
        // Servo camera command
        //=========================================================================================
        // In this part we are going to saturate the control
        // For simplicity we did already in the previous step
    ...
    void publish_default_command() {
        auto message_default = custom_interface_msgs::msg::ServoCameraCommand();
        message_default.servo_camera_angle_1 = 90;
        message_default.servo_camera_angle_2 = 90;
        RCLCPP_INFO(this->get_logger(), "Publishing default command: servo_command_1: '%d', servo_command_2: '%d'", message_default.servo_camera_angle_1, message_default.servo_camera_angle_2);
        publisher_->publish(message_default);
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<ServoCameraControl>();
